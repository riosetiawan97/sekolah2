...
function simpan_galeri(){
		...
		if(!empty($_FILES['filefoto']['name']))
		{
			if ($this->upload->do_upload('filefoto'))
			{
					...
					$this->m_galeri->simpan_galeri($judul,$album,$user_id,$user_nama,$gambar,$slider);
	            ...
	            if(!empty($_FILES['filefoto']['name']))
	            {
	                if ($this->upload->do_upload('filefoto'))
	                {
	                       ...
	                       /*  $config['quality']= '60%';
							...
							$user_id=$p['pengguna_id'];
							$user_nama=$p['pengguna_nama'];
							$this->m_galeri->update_galeri($galeri_id,$judul,$album,$user_id,$user_nama,$gambar,$slider);
	                    ...
	                    redirect('admin/galeri');
							...
							$user_id=$p['pengguna_id'];
							$user_nama=$p['pengguna_nama'];
							$this->m_galeri->update_galeri_tanpa_img($galeri_id,$judul,$album,$user_id,$user_nama,$slider);
							echo $this->session->set_flashdata('msg','info');
							redirect('admin/galeri');
	            } 
